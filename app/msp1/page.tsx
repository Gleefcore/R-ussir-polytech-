...
export default function MSP1Page() {
  ...
  const subjects = getSubjectsByLevelAndSemester('MSP1', semester);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <div className="flex items-center gap-3 mb-3">
            <div className="p-2 rounded-xl bg-poly-cyan/15 border border-poly-cyan/30">
              <GraduationCap className="w-6 h-6 text-sky-600 dark:text-poly-cyan" />
            </div>
            <span className="text-sky-600 dark:text-poly-cyan font-mono text-sm tracking-widest uppercase font-bold">
              1ère Année Ingénieur
            </span>
          </div>
          <h1 className="text-4xl font-black text-slate-900 dark:text-white mb-2">
            MSP1 <span className="text-gold-gradient">— Matières & Ressources</span>
          </h1>
        ...
        <p className="text-slate-400 dark:text-white/20 text-xs mt-8 text-center font-medium">
          {subjects.length} matières — Semestre {semester} — MSP1
        </p>

        <StudentMarquee level="MSP1" />
      </div>
    </div>
  );
}
